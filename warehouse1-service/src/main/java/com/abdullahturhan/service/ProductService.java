...
public class ProductService {
    ...
    public void takeOutProduct(TakeOutProductDto productDto) throws ProductNotFoundException, QuantityNotEnoughException {
        ...
        if (product.getQuantity() < productDto.getQuantity()){
            ...
            kafkaProducer.publish("thresholdValue_error_warehouse1",payload);
            log.info(String.format("Event sent to central with this payload: %s",payload));
            throw new QuantityNotEnoughException(String.format("Product has not enough quantity but restock request sent" +
                    " to central warehouse: %d",product.getQuantity()));
        }
    ...
    public Product getProductByName(ProductRestockDto productRestockDto) throws ProductNotFoundException {
        return productRepository.findByName(productRestockDto.getName())
                .orElseThrow(()-> new ProductNotFoundException(
                        String.format("Product not found by this name= %s",productRestockDto.getName())));
